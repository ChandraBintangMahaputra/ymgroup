...
const UserEnrollCountCard = () => {
  ...
  // Fetch user enroll count only when userId is available
  useEffect(() => {
    if (userId) {
      const fetchUserCount = async () => {
        try {
          const response = await fetch(
            `${import.meta.env.VITE_PREFIX_BACKEND}/api/dashboard/user-enroll/${userId}`
          );
          const data = await response.json();
          if (data.success && data.status === 200) {
            setTotalusers(data.data[0].total_course);
          }
        } catch (error) {
  ...
  return (
    ...
    >
      ...
      <h2 className="text-xl font-semibold mt-6 text-center">Total Enroll Course</h2>
    </div>
  );
};

export default UserEnrollCountCard;
