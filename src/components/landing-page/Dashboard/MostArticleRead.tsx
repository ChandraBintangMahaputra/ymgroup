...
const MostArticle: React.FC = () => {
  const [data, setData] = useState<{ title: string; total_visitor: string }[]>([]);
  const {darkMode} = useDarkMode()

  useEffect(() => {
    fetch(`${import.meta.env.VITE_PREFIX_BACKEND}/api/dashboard/most-article-visitor`)
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: 'Total Amount',
        data: data.map(item => parseInt(item.total_visitor, 10)),
        backgroundColor: [
            '#FF6384', // Red
            '#36A2EB', // Blue
            '#FFCE56', // Yellow
  ...
  const chartOptions: ChartOptions<'bar'> = {
    ...
    scales: {
      y: {
        beginAtZero: true,
        suggestedMax: Math.max(...data.map(item => parseInt(item.total_visitor, 10))) * 1.1,
        ticks: {
