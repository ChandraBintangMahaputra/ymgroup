...
const MostCreatorArticleMonth: React.FC = () => {
  const [data, setData] = useState<{ name: string; total_article: string }[]>([]);
  const {darkMode} = useDarkMode()

  useEffect(() => {
    fetch(`${import.meta.env.VITE_PREFIX_BACKEND}/api/dashboard/most-created-article-by-month`)
  ...
  return (
    ...
    >
      <p className="absolute top-2 left-4 font-semibold mb-5">
        5 Most Creator Article This Month
      </p>
...
export default MostCreatorArticleMonth;
