...
const MostCreatorCourseMonth: React.FC = () => {
  const [data, setData] = useState<{ name: string; total_course: string }[]>([]);
  const {darkMode} = useDarkMode()

  useEffect(() => {
    fetch(`${import.meta.env.VITE_PREFIX_BACKEND}/api/dashboard/most-created-course-by-month`)
      .then(response => response.json())
      .then(result => {
        console.log("lahh", result)
        if (result.success) {
          setData(result.data);
        }
      })
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: 'Total Amount',
        data: data.map(item => parseInt(item.total_course)),
        backgroundColor: [
            '#FF6384', // Red
            '#36A2EB', // Blue
            '#FFCE56', // Yellow
  ...
  const chartOptions: ChartOptions<'bar'> = {
    ...
    scales: {
        y: {
          beginAtZero: true,
          suggestedMax: Math.max(...data.map(item => parseInt(item.total_course, 10))) * 1.1,
          ticks: {
  ...
  return (
    ...
    >
      ...
      5 Most Creator Course This Month
      </p>
...
export default MostCreatorCourseMonth;
