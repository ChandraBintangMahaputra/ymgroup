...
const Register = () => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (!data.email.endsWith(emailDomain)) {
      toast.error("Gunakan email IPB");
      return;
    }
  
    try {
      const emailCheckResponse = await fetch(
        `${import.meta.env.VITE_PREFIX_BACKEND}/api/master/check-email/${data.email}`
      );
      const emailCheckResult = await emailCheckResponse.json();
  
      if (emailCheckResponse.status === 200 && emailCheckResult.message === "Record Found") {
        toast.error("Email is already registered. Please use a different email.");
        return;
      }
  
      if (emailCheckResponse.status === 200 && emailCheckResult.error.message === "Record not found") {
        ...
        // Process token only if it is provided
        if (data.token) {
        ...
        // Append the remaining form fields
        if (data.name) formData.append("name", data.name);
        if (data.email) formData.append("email", data.email);
        if (data.password) formData.append("password", data.password);
        if (data.date_in) formData.append("date_in", data.date_in.toISOString());
        if (data.gender) formData.append("gender", data.gender);
        if (data.position) formData.append("position", data.position);
        if (data.faculty) formData.append("faculty", data.faculty);
        if (data.prodi) formData.append("prodi", data.prodi);
        formData.append("institution", "IPB University");
  
        // Submit the form
        const response = await axios.post(
          `${import.meta.env.VITE_PREFIX_BACKEND}/api/auth/register-ipb`,
          formData,
    ...
    <main className="pt-[4.75rem] lg:pt-[5.25rem] overflow-hidden mt-10">
      ...
      >
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-5">Daftar Di Sini</h2>
          <p className="mb-5">
            Jika tidak miliki email IPB, silahkan daftar
            <span>
              <a
                href="/#/register-non-ipb"
                className="pl-1 text-blue-500 hover:text-blue-800"
              >
                di sini
              </a>
            </span>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div className="mb-1">
              <label className="block text-gray-700 font-medium mb-2">
                Email IPB<span className="text-red-500 ml-1">*</span>
              </label>
              <div className="flex items-center border border-gray-300 rounded-md">
            ...
            <div className="mb-1">
              ...
              <div className="flex items-center rounded-md">
                ...
                <Controller
                  name="institution"
                  control={control}
                  defaultValue="IPB University"
                  render={({ field }) => (
                    <input
                      {...field}
                      type="text"
                      value="IPB University"
                      readOnly
                      className="w-full p-2 bg-gray-100 text-gray-700"
                    />
                  )}
                />
              </div>
            ...
            <div className="mb-1">
              ...
              {errors.prodi && (
                <p className="text-red-500 text-xs mt-1">Prodi Wajib Diisi</p>
              )}
            </div>
            <div className="mb-1">
              <label className="block text-gray-700 font-medium mb-2">
