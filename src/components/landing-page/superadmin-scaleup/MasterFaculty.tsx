...
const MasterFaculty = () => {
  ...
  const [selectedData, setSelectedData] = useState<Faculty | null>(null);
  const [initialData, setInitialData] = useState<Faculty | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(
        `${import.meta.env.VITE_PREFIX_BACKEND}/api/master/get-faculty`
      );
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.delete(
            `${import.meta.env.VITE_PREFIX_BACKEND}/api/master/delete-faculty/${id}`
          );
  ...
  const handleEditClick = (item: Faculty) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const updatedFields: Partial<Record<keyof Faculty, string | number>> = {};

    for (const key in selectedData) {
      const newValue = selectedData[key as keyof Faculty];
      const oldValue = initialData[key as keyof Faculty];
      if (newValue !== oldValue) {
        updatedFields[key as keyof Faculty] =
    ...
    if (Object.keys(updatedFields).length > 0) {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_PREFIX_BACKEND}/api/master/edit-faculty/${
            selectedData.id
          }`,
          {
            method: "PUT",
  ...
  const handleAddFacultySubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_PREFIX_BACKEND}/api/master/add-faculty`,
        { description: newFacultyDescription },
        { headers: { "Content-Type": "application/json" } }
      );

      if (response.data.success) {
        Swal.fire("Success", "Faculty added successfully", "success").then(() => {
          window.location.reload();
        });
      } else {
        ...
        {isModalOpen && selectedData && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className={`rounded-lg m-6 p-4 max-w-lg w-full ${darkMode ? "bg-gray-600" : "bg-white"}`}>
              <h2 className="text-lg font-semibold mb-4">Edit Faculty</h2>
