...
interface discuss {
  discuss_id: number;
  discuss_date_created: string;
  description: string;
  discuss_category: string;
  discuss_creator: string;
}

const ReportDiscuss = () => {
  const { darkMode } = useDarkMode();
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const [data, setData] = useState<discuss[]>([]); // Ensure data is always an array
  ...
  const [selectedData, setSelectedData] = useState<discuss | null>(null);
  const [initialData, setInitialData] = useState<discuss | null>(null);

  const [userId, setUserId] = useState<string | null>(null);

  const [totalDiscuss, setTotalDiscuss] = useState<number | null>(null)
    ...
    const fetchData = async () => {
        try {
          const response = await fetch(
            ...
            }/api/discuss/count-discuss`
          );
          const result = await response.json();
          if (result.success && Array.isArray(result.data)) {
            setTotalDiscuss(result.data[0].total_discuss); // Make sure it's an array
          } else {
  ...
  useEffect(() => {
    if(userId){
      const fetchData = async () => {
        try {
          const response = await fetch(
            `${import.meta.env.VITE_PREFIX_BACKEND}/api/discuss/get-discuss-superadmin`
          );
          const result = await response.json();
          if (result.success && Array.isArray(result.data)) {
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.delete(
            `${import.meta.env.VITE_PREFIX_BACKEND}/api/discuss/delete-discuss/${id}`
          );
  ...
  const filteredData = data.filter((item) =>
    item.discuss_creator.toLowerCase().includes(searchTerm.toLowerCase()) ||
    item.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
    item.discuss_category.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleEditClick = (item: discuss) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const updatedFields: Partial<Record<keyof discuss, string | number>> = {};

    for (const key in selectedData) {
      const newValue = selectedData[key as keyof discuss];
      const oldValue = initialData[key as keyof discuss];
      if (newValue !== oldValue) {
        updatedFields[key as keyof discuss] =
    ...
    if (Object.keys(updatedFields).length > 0) {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_PREFIX_BACKEND}/api/discuss/edit-discuss/${
            selectedData.discuss_id
          }`,
          {
            method: "PUT",
            headers: {
      ...
      <div className="w-full flex-grow lg:w-auto lg:flex-grow lg:flex lg:flex-col">
        ...
        <main className="p-4 overflow-x-auto lg:overflow-y-auto">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-2xl font-semibold">Discuss Report</h1>
          </div>
          ...
          <div className={`shadow-lg rounded w-60 mb-7 text-center p-4 ${darkMode ? "bg-gray-500 text-white" : "bg-green-500 text-white"}`}>
                <h3 className="text-2xl">Total Discuss: {totalDiscuss}</h3>
            </div>
          <div
                  ...
                  {paginatedData.length > 0 ? (
                    paginatedData.map((item, index) => (
                      <>
                        <tr
                          key={item.discuss_id}
                          onClick={() => handleRowClick(item.discuss_id)}
                        ...
                        >
                          ...
                          <td className="border px-2 py-1">
                            {item.discuss_creator}
                          </td>
                          <td className="border px-2 py-1">
                            {item.discuss_category}
                          </td>
                          <td className="border px-2 py-1">
                            {item.discuss_date_created}
                          </td>
                          <td className="border px-2 py-1">
                            <div className="flex space-x-2">
                              ...
                              <button
                                ...
                                onClick={() =>
                                    navigate(`/report-comment/${item.discuss_id}`)
                                  }
                              >
                                <IoIosPaper />
                              </button>
                              <button
                                ...
                                onClick={() => handleDelete(item.discuss_id)}
                              >
                                <FaTrash />
                              </button>
                            </div>
      ...
      {isModalOpen && selectedData && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className={`rounded-lg m-6 p-4 max-w-lg w-full ${darkMode ? "bg-gray-600" : "bg-white"}`}>
              <h2 className="text-lg font-semibold mb-4">Edit Discuss</h2>
              <form onSubmit={handleSave}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="mb-4">
                    <label className="block text-sm font-medium">Discuss</label>
                    <input
                      type="text"
                      name="description"
