...
interface reply_comment {
  reply_comment_id: number;
  reply_comment_date_created: string;
  description: string;
  reply_comment_creator: string;
}
...
const ReportReplyComment = () => {
  ...
  const [data, setData] = useState<reply_comment[]>([]); // Ensure data is always an array
  const [searchTerm, setSearchTerm] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [expandedRow, setExpandedRow] = useState<number | null>(null); // Track expanded row
  const itemsPerPage = 5;
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedData, setSelectedData] = useState<reply_comment | null>(null);
  const [initialData, setInitialData] = useState<reply_comment | null>(null);

  const [userId, setUserId] = useState<string | null>(null);

  const [totalreplycomment, setTotalreplycomment] = useState<number | null>(null)
    ...
    const fetchData = async () => {
        try {
          const response = await fetch(
            ...
            }/api/discuss/count-reply-comment-superadmin/${id}`
          );
          const result = await response.json();
          if (result.success && Array.isArray(result.data)) {
            setTotalreplycomment(result.data[0].total_reply_comment); // Make sure it's an array
          } else {
  ...
  useEffect(() => {
    if(userId){
      const fetchData = async () => {
        try {
          const response = await fetch(
            `${import.meta.env.VITE_PREFIX_BACKEND}/api/discuss/get-reply-comment-superadmin/${id}`
          );
          const result = await response.json();
          if (result.success && Array.isArray(result.data)) {
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.delete(
            `${import.meta.env.VITE_PREFIX_BACKEND}/api/discuss/delete-reply-comment/${id}`
          );
  ...
  const filteredData = data.filter((item) =>
    item.reply_comment_creator.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleEditClick = (item: reply_comment) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const updatedFields: Partial<Record<keyof reply_comment, string | number>> = {};

    for (const key in selectedData) {
      const newValue = selectedData[key as keyof reply_comment];
      const oldValue = initialData[key as keyof reply_comment];
      if (newValue !== oldValue) {
        updatedFields[key as keyof reply_comment] =
    ...
    if (Object.keys(updatedFields).length > 0) {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_PREFIX_BACKEND}/api/discuss/edit-reply-comment/${
            selectedData.reply_comment_id
          }`,
          {
            method: "PUT",
            headers: {
      ...
      <div className="w-full flex-grow lg:w-auto lg:flex-grow lg:flex lg:flex-col">
        ...
        <main className="p-4 overflow-x-auto lg:overflow-y-auto">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-2xl font-semibold">Reply Comment Report</h1>
          </div>
          ...
          <div className={`shadow-lg rounded w-60 mb-7 text-center px-2 py-4 ${darkMode ? "bg-gray-500 text-white" : "bg-green-500 text-white"}`}>
                <h3 className="text-2xl">Total Reply Comment: {totalreplycomment}</h3>
            </div>
          <div
                ...
                <tbody>
                  {paginatedData.length > 0 ? (
                    paginatedData.map((item, index) => (
                      <>
                        <tr
                          key={item.reply_comment_id}
                          onClick={() => handleRowClick(item.reply_comment_id)}
                        ...
                        >
                          ...
                          <td className="border px-2 py-1">
                            {item.reply_comment_creator}
                          </td>
                          <td className="border px-2 py-1">
                            {item.reply_comment_date_created}
                          </td>
                          <td className="border px-2 py-1">
                            <div className="flex space-x-2">
                              ...
                              <button
                                ...
                                onClick={() => handleDelete(item.reply_comment_id)}
                              >
                                <FaTrash />
                              </button>
                            </div>
                        ...
                        {expandedRow === item.reply_comment_id && (
                          <tr>
      ...
      {isModalOpen && selectedData && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className={`rounded-lg m-6 p-4 max-w-lg w-full ${darkMode ? "bg-gray-600" : "bg-white"}`}>
              <h2 className="text-lg font-semibold mb-4">Edit Reply Comment</h2>
              <form onSubmit={handleSave}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="mb-4">
                    <label className="block text-sm font-medium"> Reply Comment</label>
                    <input
                      type="text"
                      name="description"
