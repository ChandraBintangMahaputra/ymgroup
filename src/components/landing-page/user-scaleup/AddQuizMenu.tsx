...
const AddCourseQuiz = () => {
  ...
  const [editedCourse, setEditedCourse] = useState<quizCourse | null>(null); // Holds the course being edited

  const handleAddCourseSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_PREFIX_BACKEND}/api/course/add-quiz-course`,
  ...
  const handleEditCourse = async (id: number, description: string) => {
    try {
      const response = await axios.put(
        ...
        }/api/course/edit-quiz-course/${id}`,
        { description }
      );
      if (response.data.success) {
  ...
  const handleDeleteCourse = async (id: number) => {
    try {
      const response = await axios.delete(
        `${
          import.meta.env.VITE_PREFIX_BACKEND
        }/api/course/delete-quiz-course/${id}`
      );
      if (response.data.success) {
  ...
  const fetchCourses = async () => {
    try {
      const response = await axios.get(
        `${
          import.meta.env.VITE_PREFIX_BACKEND
        }/api/course/list-quiz-course/${courseId}`
      );
  ...
  const openEditModal = (course: quizCourse) => {
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      const response = await axios.put(
        `${
          import.meta.env.VITE_PREFIX_BACKEND
        }/api/course/update-quiz-course-order`,
        updatePayload
      );
      if (response.data.success) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-grow">
        ...
        <main className="p-4">
          <h1 className="text-2xl font-bold mb-4">Add quiz Course</h1>
                      ...
                      >
                        {(provided) => (
                          <div
                            ...
                            onClick={() =>
                              navigate(`/list-question/${course.id}`)
                            }
                          >
      ...
      {editedCourse && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          ...
          >
            <h2 className="text-lg font-semibold mb-4">Rename quiz</h2>
            <form
              onSubmit={(e) => {
                e.preventDefault();
