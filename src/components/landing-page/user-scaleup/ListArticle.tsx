...
const ListArticle = () => {
      ...
      const fetchData = async () => {
        try {
          const response = await fetch(
            ...
            }/api/article/all-article/${userId}`
          );
  ...
  const handlePublish = async (id: number) => {
    Swal.fire({
      ...
      text: "Article akan dapat dilihat oleh publik",
      icon: "warning",
      showCancelButton: true,
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.put(
            `${
              import.meta.env.VITE_PREFIX_BACKEND
            }/api/article/publish-article/${id}`
          );
          if (response.data.success) {
            Swal.fire("Berhasil!", "Article berhasil dipublish.", "success")
          } else {
            Swal.fire("Error", "Gagal publish article.", "error");
          }
        } catch (error) {
          console.error("Error publish article:", error);
          Swal.fire("Error", "Terjadi kesalahan saat publish article.", "error");
        }
      }
    });
  };
                        ...
                        <td className="border px-2 py-1">
                          <div className="flex space-x-2">
                            ...
                            <button
                              ...
                              onClick={() => handlePublish(item.article_id)}
                            >
                              Publish
                            </button>
                          </div>
